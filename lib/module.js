...
module.exports = function (moduleOptions) {
  ...
  const moduleRouterCollection = () => routerPaths().reduce((router, path) => {
    ...
    const rawRouter2 = process.env.ROUTER_MODE === 'TREE' ? rawConfig : treeToArray(configToRoutes(rawConfig), {}, true)
    const treatRouter = process.env.ROUTER_FILE === 'router' ? replaceComponentPath(rawRouter2, options.modulePath, moduleName) : replaceComponentPath(rawRouter, options.modulePath, moduleName)
